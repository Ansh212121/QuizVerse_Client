 ...
 const finishQuiz = useCallback(
  async (answers) => {
    ...
    try {
      ...
      // High score check
      const previousBestKey = `highscore_${config.topic}_${config.difficulty}`;
      const previousBest = parseInt(localStorage.getItem(previousBestKey) || "0");

      if (finalScore > previousBest) {
        toast.success("🎉 New highest score achieved!");
        localStorage.setItem(previousBestKey, finalScore.toString());
      }
      ...
      await axios.post(`${URL}/results/SaveQuizResults`, detailedResults, {
        headers: {
          "Content-Type": "application/json",
          "auth-token": token,
        },
