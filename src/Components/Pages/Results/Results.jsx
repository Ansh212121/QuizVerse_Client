...
function Results() {
  ...
  const calculateStats = () => {
    ...
    const totalCorrect = recentResults.reduce((sum, r) => sum + r.score, 0);
    const totalQuestions = recentResults.reduce((sum, r) => sum + r.totalQuestions, 0);
    const avgAccuracy = (totalCorrect / totalQuestions) * 100;
    const totalTime = recentResults.reduce((sum, r) => sum + r.timeTaken, 0);
  ...
  const getBestTopic = () => {
    ...
    return { name: bestTopic, accuracy: highestAvg.toFixed(1) };
  };

  const calculateImprovement = () => {
    if (recentResults.length < 2) return "N/A";
    const sorted = [...recentResults].sort((a, b) => new Date(a.date) - new Date(b.date));
    const segmentSize = Math.max(1, Math.floor(sorted.length * 0.2));
    const first = sorted.slice(0, segmentSize);
    const last = sorted.slice(-segmentSize);
    const firstAvg = (first.reduce((sum, r) => sum + r.score / r.totalQuestions, 0) / first.length) * 100;
    const lastAvg = (last.reduce((sum, r) => sum + r.score / r.totalQuestions, 0) / last.length) * 100;
